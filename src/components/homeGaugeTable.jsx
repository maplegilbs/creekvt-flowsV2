            ...
            {gaugeFetchAndMergeStatus === "success" &&
                <div className={`${styles["info__container"]} ${homeTableStyles["info__container"]}`}>
                    <table className={`${styles["flows__table"]} ${homeTableStyles["flows__table"]}`}>
                        ...
                        <tbody>
                            {mergedRiverData &&
                                sortByRunning(mergedRiverData).map(filterGauges).length > 0 ? sortByRunning(mergedRiverData).map(filterGauges).length : <><tr><br/></tr><tr><td colSpan={20}>No Rivers Currently Running</td></tr> </>
                            }
                        </tbody>
                    </table>
                </div>
