...
const isRecent = (new Date() - new Date(levelReport.tripDateTime)) < (24 * 60 * 60 * 1000);
    ...
    return (
        <tr className={`${styles["visuals-row"]}`}>
            ...
            <td className={`mobile-hide ${styles["gauge-cell"]}`}>{levelReport.gauge1ReadingTime ? `${formatDateTime(levelReport.gauge1ReadingTime).numericDate} ${formatDateTime(levelReport.gauge1ReadingTime).time}${formatDateTime(levelReport.gauge1ReadingTime).amPm}`:''}</td>
            ...
            <td className={`mobile-hide ${styles["gauge-cell"]}`}>{levelReport.gauge2ReadingTime? `${formatDateTime(levelReport.gauge2ReadingTime).numericDate} ${formatDateTime(levelReport.gauge1ReadingTime).time} ${formatDateTime(levelReport.gauge1ReadingTime).amPm}` : '' }</td>
